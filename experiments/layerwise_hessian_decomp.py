...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='layerwise hessian decomposition')
...
if __name__ == '__main__':
    ...
    logger = logging.getLogger('layerwise_hessian_decomp')
    logging_file = osp.join(log_dir, 'layerwise_hessian_decomp.log')
    ...
    for layer_index, (layer_name, _) in enumerate(model.named_parameters()):
        ...
        logger.info('hessian decomposition finished')
        ...
        image_path = osp.join(images_dir, '{}_hessian_decomp.png'.format(layer_name))
        plt.savefig(image_path, cpi=args.dpi)
        logger.info('Saved plot for layer {} ...'.format(layer_name))