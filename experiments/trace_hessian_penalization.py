# This code is to penalize hessian of loss of a NN
...
def parse_args():
    ...
    # model_choices = ['AlexNet', 'VGG11', 'VGG11_bn', 'VGG13', 'VGG13_bn', 'VGG16', 'VGG16_bn', 'VGG19', 'VGG19_bn', 'ResNet18', 'DenseNet3_40', 'LeNet', 'MobileNet', 'FullyConnected']
    ...
    parser.add_argument('--sp',
                        type=int,
                        default=default_sp,
                        help='scale point is epoch after which hessian has to be scaled, default={}'.format(default_sp)
                        )
        ...
        with torch.enable_grad():
            for batch, truth in dataloaders['train']:
                ...
                if epoch >= sp:
                    ...
                    grads = torch.autograd.grad(loss, xs, create_graph=True)  # first order gradient
                    grads2 = get_second_order_grad(grads, xs, device)
                    sum_dl2_dw2 = sum([g.sum for g in grads2])
                    loss = loss + sf * sum_dl2_dw2
                    print(sum_dl2_dw2)
                loss.backward()
                optimizer.step()
...
if __name__ == '__main__':
    ...
    if args.dataset == 'MNIST':
        ...
        train_data = datasets.STL10(osp.join(args.dataset_root, 'STL10'), split='train', transform=train_transform, download=True)
        test_data = datasets.STL10(osp.join(args.dataset_root, 'STL10'), split='test', transform=train_transform, download=True)
    else:
