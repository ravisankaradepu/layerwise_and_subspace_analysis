# New hessian trace where the idea of Hv with v as canonical vecotr is implemented
#!/usr/bin/env python3
...
def get_second_order_grad(grads, xs):
    ...
    for j, (grad, x) in enumerate(zip(grads, xs)):
        ...
        print(x.size())
        for i in tqdm(range(len(x.reshape(-1)))):
            can_vec = torch.zeros_like(grad.flatten())
            can_vec[i] = 1
            a=torch.autograd.grad(grad.flatten(),x, grad_outputs = can_vec,retain_graph = True)
            trace += a[0].reshape(-1)[i]
    return trace
    ...
    for epoch in range(num_epochs):
        ...
        with torch.enable_grad():
            for step, (batch, truth) in enumerate(dataloaders['train']):
                ...
                output = model(batch)
                loss = criterion(output, truth)
                if train_type == 'ht' and step % args.freq == 0 and epoch >= args.sp:
                    xs = optimizer.param_groups[0]['params']
                    grads = torch.autograd.grad(loss,xs,create_graph = True)                     
                    trace_loss = get_second_order_grad(grads,xs)
                    trace_loss = get_trace_loss(model, batch, truth, criterion, args.hi)
                    loss = loss + args.sf * trace_loss
                    # loss = loss + args.sf / trace_loss
                    # if epoch >= args.sp:
                    #     loss = loss + args.sf * trace_loss
                    # else:
...
if __name__ == '__main__':
    ...
    for rerun in range(args.reruns):
        for train_type in ['ht','ord']:
            ...
            run_acc_list, run_loss_list = train(model,
                                                optimizer,
                                                # scheduler,
                                                dataloaders,
                                                criterion,
            ...
            if train_type == 'ord':
                all_acc['train']['ord'].append(run_acc_list['train'])
                all_loss['train']['ord'].append(run_loss_list['train'])
                all_acc['test']['ord'].append(run_acc_list['test'])
                all_loss['test']['ord'].append(run_loss_list['test'])
            elif train_type == 'ht':
                all_acc['train']['ht'].append(run_acc_list['train'])
                all_loss['train']['ht'].append(run_loss_list['train'])
                all_acc['test']['ht'].append(run_acc_list['test'])
                all_loss['test']['ht'].append(run_loss_list['test'])
            else:
                raise Exception('Unknown train_type: {}'.format(train_type))

        assert len(all_acc['train']['ord'][0]) == len(all_acc['train']['ord'][-1])
        assert len(all_loss['train']['ord'][0]) == len(all_loss['train']['ord'][-1])
        assert len(all_acc['test']['ord'][0]) == len(all_acc['test']['ord'][-1])
        assert len(all_loss['test']['ord'][0]) == len(all_loss['test']['ord'][-1])
