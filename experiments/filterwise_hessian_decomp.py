...
from hessian import FilterHessian
...
def parse_args():
    ...
    parser.add_argument('-f', '--filter', type=int, required=True, help='filter number of layer to be analyzed')
...
if __name__ == '__main__':
    ...
    logger = logging.getLogger('{}_{}_filter_decomp'.format(args.layer, args.filter))
    logging_file = osp.join(log_dir, '{}_{}_filter_decomp.log'.format(args.layer, args.filter))
    ...
    if args.model == 'VGG11_bn' or args.model == 'ResNet18' or args.model == 'DenseNet3_40':
    ...
    Hess = FilterHessian(crit='CrossEntropyLoss',
                        loader=loader,
                        device=device,
                        model=model,
                        num_classes=C,
    ...
    G = FilterHessian(crit='CrossEntropyLoss',
                        loader=loader,
                        device=device,
                        model=model,
                        num_classes=C,
    ...
    image_path = osp.join(images_dir, '{}_{}_filter_decomp.png'.format(args.layer, args.filter))
